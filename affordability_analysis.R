...
#Define function to calculate affordability flags 
...
affdt_1215_1BU <- affdt_1215[ multiple_benu == 0, ]

#Flag for is HRP - can also be used to collapse dataset down to household level for household level variables
affdt_1215_1BU[ , is_HRP := ifelse( PERSON == HRPNUM, 1, 0) ]

#Add 2 year hhd weight
#affdt_1315_1BU[ , hhweight_2 := GS_NEWHH/2]
affdt_1215_1BU[ , hhweight_3 := GS_NEWHH/3]

affdt_1215_1BU[ , GVTREGN := factor(GVTREGN, labels = c("North East", "North West", "Yorkshire and Humber", "East Midlands",
                                                        "West Midlands", "East", "London", "South East", "South West"))]

#Flag for renter
affdt_1215_1BU[ , prs_renter := ifelse(PTENTYP2 == 3 | PTENTYP2 == 4, 1, 0)]

#Flag for eligible for Starter Homes
affdt_1215_1BU[ , eligible_starter := ifelse(HDAGE <4, 1, 0)]


#~~~~~~~~~~~~~~~~~~~~~~~~ experimenting with survey package
...
deshhd_1215 <- svydesign(ids = ~1, weights = ~ hhweight_3, data = affdt_1215_1BU[affdt_1215_1BU$is_HRP & 
                                                                                   (is.na(affdt_1215_1BU$hhweight_3) == F), ])

#Private renters who can't afford LQ Starter Homes, with SEs
ftable(svyby(formula = ~aff_starterlq_noincben, by = ~GVTREGN, design = subset(deshhd_1215, prs_renter == 1), FUN = svymean))

#Private renters who are eligible for Starter Homes but can't afford shared ownership, with SEs
ftable(svyby(formula = ~aff_shared_noincben, by = ~GVTREGN, design = subset(deshhd_1215, (prs_renter == 1 & eligible_starter ==1)), 
             FUN = svymean))
...
#t_shared_afford_el[,1]<- NULL
#t_shared_afford_el$Region <- rownames(t_shared_afford_el)
#### Private renters who can't afford ownership
affdt_1215_1BU[ ( prs_renter & is_HRP), sjt.xtab(aff_starterlq_noincben, GVTREGN,
...
affdt_1215_1BU[ ( prs_renter & is_HRP), sjt.xtab(aff_starterm_noincben, GVTREGN,
...
affdt_1215_1BU[ ( prs_renter & is_HRP), sjt.xtab((aff_htb_noincben), GVTREGN,
...
affdt_1215_1BU[ ( prs_renter & is_HRP), sjt.xtab(aff_shared_noincben, GVTREGN,
